...
const EditarRutaTab = () => {
  ...
  const abrirModalEntrega = (key: string) => {
  ...
  return (
    <div className={classes.root}>
      ...
      <ModalCustom isOpen={isOpenModalEntrega} style={{ width: 400 }}>
        ...
        <div>
          ...
          <input
            ...
            value={entregaRut}
            onInput={(e) => setEntregaRut(formatRut((e.target as HTMLInputElement).value))}
          />
        </div>
        <div>
          <label>Nombre</label>
        ...
        <div style={{ display: "flex", justifyContent: "space-around", width: "100%" }}>
          <Button onClick={() => setIsOpenModalEntrega(false)}>Cancelar</Button>
          <Button onClick={marcarEntregado}>Aceptar</Button>
        </div>
      </ModalCustom>
        ...
        {(key: string) => (
          <>
            ...
            <button className={classes.button + " " + classes.buttonEntregado}
              onDoubleClick={() => { abrirModalEntrega(key); }}>
              Marcar entregado
            </button>
